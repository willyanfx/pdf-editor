...
export const STANDARD_FAMILIES: StandardFontFamily[] = ["Helvetica", "Times", "Courier"];
...
export function ensureFontLoaded(family: FontFamily): void {
  ...
  const href = `https://fonts.googleapis.com/css2?family=${encoded}:ital,wght@0,400;0,700;1,400;1,700&display=swap`;
...
export async function waitForFonts(families: FontFamily[]): Promise<void> {
  ...
  await Promise.all(
    googleFamilies.flatMap((family) =>
      ["400 16px", "700 16px", "italic 400 16px", "italic 700 16px"].map((spec) =>
