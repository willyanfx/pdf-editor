export const meta = {
  ...
  description:
    "Audit the PDF editor against Adobe Acrobat features; draft a gap-closing plan for every feature that is absent or partial",
  whenToUse:
    "Run to measure the codebase against Acrobat's marketed feature set and get an implementation plan for each gap.",
  phases: [
    {
      title: "Audit",
      detail: "one read-only agent per feature: implemented / partial / absent + file evidence",
    },
    {
      title: "Plan",
      detail: "one architect agent per non-implemented feature: files, libraries, approach",
    },
  ],
};
...
const FEATURES = [
  {
    ...
    seedHint:
      "Text editing implemented (textLayer.ts, ExistingTextLayer.tsx, RichTextEditor.tsx, exportPdf.ts). Placing new images implemented (openFiles.ts, EditBox.tsx). Swapping/replacing an image already embedded in the source PDF appears ABSENT.",
  },
  {
    ...
    seedHint:
      "Appears absent. File picker only accepts application/pdf; no docx/xlsx/pptx/image-to-pdf conversion pipeline or libraries.",
  },
  {
    key: "sign",
    ...
    seedHint:
      'Partial: an "Add Image / Signature" overlay lets you place a PNG/JPEG (e.g. a photo of a signature). No freehand signature canvas and no cryptographic/certificate-based digital signing.',
  },
  {
    ...
    seedHint:
      "Appears absent. Viewer is read-only (tanstack virtual scroll). No thumbnail/page panel, no reorder, no delete. pdf-lib supports removePage/insertPage but they are not called.",
  },
  {
    key: "merge-split",
    ...
    seedHint:
      "Appears absent. No multi-PDF picker, no PDFDocument.copyPages usage, no page-range split UI.",
  },
  {
    key: "rotate-crop",
    ...
    seedHint:
      "Appears absent. No page rotate/crop controls. pdf-lib setRotation/setCropBox/setMediaBox not used. BottomBar only has zoom.",
  },
  {
    key: "annotate",
    label: "Annotate PDFs",
    ...
    seedHint:
      "Appears absent. No highlight/underline/comment/ink tools. renderAnnotationLayer={false}. Only a plain black-border rectangle shape (addRectangle) exists, drawn via page.drawRectangle on export.",
  },
  {
    key: "compress",
    ...
    seedHint:
      "Appears absent. exportPdf.ts calls pdfDoc.save() with no options; no image downsampling, object streams, or compress command.",
  },
  {
    key: "ocr",
    ...
    seedHint:
      "OCR extraction/editing implemented (ocr.ts with tesseract.js worker, OcrLayer.tsx region OCR, full-page + image OCR, recognized lines become editable cover-text edits). Text SEARCH / Find bar appears ABSENT.",
  },
];
...
const AUDIT_SCHEMA = {
  ...
  properties: {
    ...
    status: {
      ...
      description:
        "implemented = the full Acrobat claim is met; partial = some of it works but something specific is missing; absent = no real support.",
    },
    missing: {
      type: "string",
      description:
        "If partial or absent, exactly what is missing relative to the Acrobat claim. Empty string if implemented.",
    },
    evidence: {
      type: "array",
      description:
        "Concrete code citations. For absent features, cite where it WOULD live / what proves its absence (e.g. the file picker accept filter).",
      items: {
        type: "object",
        additionalProperties: false,
        required: ["path", "note"],
        properties: {
          path: {
            type: "string",
            description:
              "Real repo-relative file path you actually inspected, e.g. src/lib/ocr.ts.",
          },
    ...
    libraries: {
      ...
      items: { type: "string" },
      description:
        "Libraries already present that this feature uses or would use (e.g. pdf-lib, tesseract.js).",
    },
...
const PLAN_SCHEMA = {
  ...
  properties: {
    ...
    effort: {
      type: "string",
      enum: ["S", "M", "L"],
      description: "Rough size: S < a day, M a few days, L a week+.",
    },
    newLibraries: {
      type: "array",
      items: { type: "string" },
      description:
        "npm packages to add. Empty if achievable with existing deps (pdf-lib, pdfjs-dist, react-pdf, tesseract.js, react-rnd, zustand).",
    },
    filesToTouch: {
      type: "array",
      items: {
        ...
        properties: {
          path: {
            type: "string",
            description: "Repo-relative path (existing file to edit or new file to create).",
          },
    ...
    approach: {
      type: "string",
      description:
        "Concise implementation approach. Name existing utilities to reuse (e.g. exportPdf.ts PDFDocument, useEditorStore edits, EditBox react-rnd pattern, ocr.ts worker).",
    },
    risks: {
      type: "array",
      items: { type: "string" },
      description: "Pitfalls, edge cases, or unknowns.",
    },
  },
};
...
const results = await pipeline(
  ...
  (audit, f) => {
    ...
    return agent(planPrompt(f, audit), {
      label: `plan:${f.key}`,
      phase: "Plan",
      schema: PLAN_SCHEMA,
      agentType: "Plan",
...
log(
  `Done: ${summary.implemented.length} implemented, ${summary.partial.length} partial, ${summary.absent.length} absent; ${summary.plansDrafted} gap plans drafted.`,
);
